...
const MenuInferior = () => {
  ...
  useEffect(() => {
    fetchOrders(); // Obtenemos las órdenes al montar el componente
  }, []);

  return (
    <div
      className="col w-100 position-fixed bottom-0 end-0 py-1"
      style={{ backgroundColor: "#1b1b1b", zIndex: "997" }}
    >
      <div style={{ paddingLeft: "4.5rem" }}>
        <div className="d-flex table-responsive m-0 p-0">
          <NavLink
            to="/ventas"
            className={({ isActive }) =>
              `col-2 col-md-1 carrito py-1 border-end border-1 ${
          ...
          {orders.length > 0 &&
            orders
              ...
              .map((order) => (
                <NavLink
                  key={order.id}
                  to={`/form_ventas/${order.id}`}
                  className={({ isActive }) =>
                    `col-auto ventana my-auto border-end py-1 border-1 ${
                      isActive ? "active-link" : ""
                    }`
                  }
                >
          ...
          <button
            ...
            className="col-auto text-center my-1 border-start border-1 position-sticky"
            style={{ right: "0px", backgroundColor: "#1b1b1b", border: "none" }}
          >
